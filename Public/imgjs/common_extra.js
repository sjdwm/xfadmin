function _zoom(obj, zimg, nocover, pn, showexif) {
    ...
    var loadCheck = function(obj) {
        if (obj.complete) {
            ...
            if (id(menuid)) {
                id(menuid).removeAttribute('top_');
                id(menuid).removeAttribute('left_');
    ...
    var showimage = function(zimg, w, h, imgw, imgh) {
        ...
        id(menuid + '_img').innerHTML = '<img id="' + zoomid + '" src="' + zimg + '" width="' + w + '" height="' + h + '" w="' + imgw + '" h="' + imgh + '" />';
...
function _zoom_page(paid, showexif) {
    var imagesrc = id('aimg_' + paid).getAttribute('zoomfile') ? id('aimg_' + paid).getAttribute('zoomfile') : id('aimg_' + paid).getAttribute('file');
